...
class HTTPServer:
    ...
    def _setup_logger(self):
        ...
        file_handler = logging.FileHandler("./cgiserver/log/basic_log.log",mode='a+', encoding='utf-8', delay=False)
        file_handler.setFormatter(file_formatter)
        logger.addHandler(file_handler)

        html_formatter = logging.Formatter(
            '<p>[%(asctime)s] - [%(levelname)s] %(message)s</p>')
        html_handler =  logging.FileHandler("./cgiserver/log/log.html",mode='a+', encoding='utf-8', delay=False)
